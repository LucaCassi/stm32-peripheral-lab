...
/* USER CODE BEGIN PV */
uint16_t AD_RES = 0;     // Luca: Analog to Digital conversion result
/* USER CODE END PV */
...
int main(void)
{
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  ...
	  // Luca: this does not seem very robust, since i call for a conversion

	  // Luca: the previous code is not super robust: AD_RES should be volatile, since I use its value in while(1) but assign it a value in HAL_ADC_ConvCpltCallback();
	  //       plus, it is likely not relevant for this application, but i may be changing AD_RES while trying to assign it and failing to do so (I do not delve into this now);
  }
  /* USER CODE END 3 */
}
